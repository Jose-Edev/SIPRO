    ...
    <div class="row">
      ...
      <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
          <h1 class="h2">Editar Salida</h1>
        </div>

        <form action="metodos/editarOrdenSalida.php" method="post" enctype="multipart/form-data">
          <div class="table-responsive small">
            <div class="mb-3">
              <label class="form-label label-large">Fecha de entrega</label>
              <input type="text" class="form-control" id="fecha_de_entrega" name="fecha_de_entrega" aria-describedby="emailHelp" value="<?php echo htmlspecialchars($requisicion_data['fecha_de_entrega']); ?>">
            </div>
            <!------------------------------------------------------ARREGLAR ORDENES DE SALIDA------------------------------------------------------>
            <!-- <div class="mb-3">
              <label class="form-label label-large">No. de Salida</label>
              <input type="text" class="form-control" id="numero_de_salida_display" name="numero_de_salida_display" aria-describedby="emailHelp" readonly>
              <button type="button" class="btn btn-primary" id="asignarNoSalida">Asignar No de salida</button>
            </div>
          </div>
          <script>
            document.getElementById('asignarNoSalida').addEventListener('click', function() {
              var xhr = new XMLHttpRequest();
              xhr.open('GET', 'metodos/AsignarSalida.php', true);
              xhr.onreadystatechange = function() {
                if (xhr.readyState == 4 && xhr.status == 200) {
                  document.getElementById('numero_de_salida_display').value = xhr.responseText; // Asignar el número de salida
                }
              };
              xhr.send();
            });
          ...
          <script>
            document.addEventListener('DOMContentLoaded', function() {
              var fechaEnvioInput = document.getElementById('fecha_de_entrega');
              if (!fechaEnvioInput.value) {
                var today = new Date();

                // Opciones para formatear la fecha en formato largo
                var options = {
                  weekday: 'long', // Nombre completo del día (ejemplo: "miércoles")
                  year: 'numeric', // Año completo
                  month: 'long', // Nombre completo del mes (ejemplo: "agosto")
                  day: 'numeric' // Día del mes
                };

                // Formatear la fecha a la cadena larga
                var todayString = today.toLocaleDateString('es-ES', options);

                // Establecer el valor en el elemento con id 'fecha_de_entrega'
