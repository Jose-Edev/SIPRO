...
$requisicion_data = $bd->getOrdenTrabajo2($requisicion_id);
$partidas = $bd->getPartidasByRequisicionId3($requisicion_id);
    ...
    <div class="row">
            ...
            <ul class="nav nav-pills flex-column">
              ...
              <li>
                <a href="" style="background-color: red;" class="nav-link d-flex align-items-center gap-2 text-white">
                  <i class="fas fa-file-alt"></i>Órdenes de Trabajo
                </a>
              </li>
            </ul>
            <hr style="color: white;">
      ...
      <main class="col-md-9 ms-sm-auto col-lg-10 px-md-4">
        <div class="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center pt-3 pb-2 mb-3 border-bottom">
          <h1 class="h2">Editar Orden de Trabajo</h1>
        </div>

        <form action="metodos/EditarOrdenT.php" method="post" enctype="multipart/form-data">
          <div class="table-responsive small">
            ...
            <div class="mb-3">
              <label class="form-label label-large">Fecha de elaboración</label>
              <input type="date" class="form-control" id="fecha_elab" name="fecha_elab" aria-describedby="emailHelp" value="<?php echo htmlspecialchars($requisicion_data['fecha_elab']); ?>">
            </div>
            <script>
              document.addEventListener('DOMContentLoaded', function() {
                var fechaEnvioInput = document.getElementById('fecha_elab');
                if (!fechaEnvioInput.value) {
                  var today = new Date();
                  var day = String(today.getDate()).padStart(2, '0');
                  var month = String(today.getMonth() + 1).padStart(2, '0');
                  var year = today.getFullYear();
                  var todayString = year + '-' + month + '-' + day;
                  fechaEnvioInput.value = todayString;
                }
              });
            </script>
            ...
            <div id="partidas-container">
              <?php foreach ($partidas as $index => $partida) : ?>
                <div class="partida mb-3">
                  ...
                  <div class="mb-3">
                    <label class="form-label label-large">Muestra</label>
                    <select class="form-select" aria-label="Default select example" id="plano" name="partidas[<?php echo $index; ?>][muestra]" aria-describedby="emailHelp">
                      <option value="SI" <?php echo $partida['muestra'] == 'SI' ? 'selected' : ''; ?>>SI</option>
                      <option value="NO" <?php echo $partida['muestra'] == 'NO' ? 'selected' : ''; ?>>NO</option>
                    </select>
                  </div>

                  <div class="mb-3">
                    <label class="form-label label-large">Sobre pieza</label>
                    <select class="form-select" aria-label="Default select example" id="plano" name="partidas[<?php echo $index; ?>][sobre_pieza]" aria-describedby="emailHelp">
                      <option value="SI" <?php echo $partida['sobre_pieza'] == 'SI' ? 'selected' : ''; ?>>SI</option>
                      <option value="NO" <?php echo $partida['sobre_pieza'] == 'NO' ? 'selected' : ''; ?>>NO</option>
                    </select>
                  </div>
                  <div class="mb-3">
                    <label class="form-label label-large">Material</label>
                    <input type="text" class="form-control" name="partidas[<?php echo $index; ?>][material]" aria-describedby="emailHelp" value="<?php echo htmlspecialchars($partida['material']); ?>">
                  </div>
                  <div class="mb-3">
                    <label class="form-label label-large">Fecha de entrega</label>
                    <input type="date" class="form-control" name="partidas[<?php echo $index; ?>][fecha_entrega]" aria-describedby="emailHelp" value="<?php echo htmlspecialchars($partida['fecha_entrega']); ?>">
                  </div>
                </div>
              <?php endforeach; ?>
              <br>
